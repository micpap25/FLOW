...
public class Player2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else if (hitstun == 0 && canCancel)
        {
            ...
            if (Input.GetKeyDown(KeyCode.UpArrow) && !isAirborne && dashTime <= 0 && !movementsTaken.Contains("jump") && !isCrouching)
            {
                if (left)
                {
        ...
        while (endlag > 0)
        {
            if (endlag <= ending - startup && endlag > ending - startup - hittime && !hasHit)
            {
                ...
                if (enemyHit != null)
                {
                    ...
                    canCancel = true;
                    combocrouching = true;
                    enemyHit.gameObject.GetComponent<Player1>().TakeDamage(damage, stun, angle, attackTypes, launch);
                    hasHit = true;
                }
            }
            else
